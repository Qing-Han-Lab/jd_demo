<style scoped lang='scss'>
.login {
  @extend .page;
  .login-content {
    flex: 1;
    width: 100%;
    padding: 25px;
    box-sizing: border-box;
    overflow: hidden;
    .login-content-inputbox {
      margin-top: 20px;
      .login-content-inputbox-item {
        margin-top: 20px;
        padding: 10px 0;
        display: flex;
        border-bottom: 1px solid #efefef;
        align-items: center;
        .login-content-inputbox-item-left {
          width: 50px;
          height: 100%;
        }
        .login-content-inputbox-item-right {
          width: 110px;
          height: 23px;
          color: $jd-textcolor;
          border-left: 1px solid $jd-backcolor;
          box-sizing: border-box;
          text-align: center;
          font-size: 14px;
          line-height: 23px;
        }
        input {
          flex: 1;
          height: 96%;
          outline: none;
          border: none;
          &::-webkit-input-placeholder {
            color: #999;
          }
        }
      }
    }
    .login-content-loginbtn {
      width: 100%;
      height: 50px;
      line-height: 50px;
      text-align: center;
      color: #fff;
      margin-top: 20px;
      background-image: linear-gradient(90deg, #fab3b3, #ffbcb3 73%, #ffcaba);
      border-radius: 30px;
      transition: background 0.3s;
      &.active {
        background: $jd-backcolor;
      }
    }
    .login-content-morelogin {
      font-size: 12px;
      color: #999;
      margin-top: 30px;
      display: flex;
      justify-content: space-between;
      align-items: center;
      .login-content-morelogin-text {
        margin: 0 8px;
      }
      &::before {
        content: "";
        flex: 1;
        height: 1px;
        background: #efefef;
      }
      &::after {
        content: "";
        flex: 1;
        height: 1px;
        background: #efefef;
      }
    }
    .login-content-moreway {
      margin: 25px 0 12px;
      display: flex;
      justify-content: center;
      align-items: center;
      .login-content-moreway-item {
        margin: 0 15px;
        display: flex;
        flex-direction: column;
        justify-content: space-between;
        align-items: center;
        font-size: 12px;
        color: rgba(0, 0, 0, 0.4);
        &:last-child .logo {
          background-image: url("data:image/png;base64,");
        }
        .logo {
          width: 48px;
          height: 48px;
          background-size: 100%;
          background-image: url("data:image/png;base64,");
        }
      }
    }
    .login-content-info {
      font-size: 13px;
      color: rgba(0, 0, 0, 0.3);
      text-align: center;
      margin-top: 20px;
      span {
        color: #4a90e2;
      }
    }
  }
}
</style>


<template>
  <div class="login">
    <Slidecheck ref="check" v-if="isShow" />
    <Title title="用户登录" />
    <div class="login-content">
      <div class="login-content-inputbox">
        <div class="login-content-inputbox-item">
          <div class="login-content-inputbox-item-left">+86</div>
          <input type="number" placeholder="请输入手机号" v-model="phoneNumber" />
        </div>
        <div class="login-content-inputbox-item">
          <input type="number" placeholder="请输入验证码" v-model="checkCodeNumber" />
          <div class="login-content-inputbox-item-right">获取验证码</div>
        </div>
      </div>
      <div class="login-content-loginbtn" :class="{active:isActive}" @click="login('h5')">登 录</div>
      <div class="login-content-morelogin">
        <div class="login-content-morelogin-text">更多登录方式</div>
      </div>
      <div class="login-content-moreway">
        <div class="login-content-moreway-item" @click="login('qq')">
          <div class="logo"></div>QQ
        </div>
        <div class="login-content-moreway-item" @click="login('weixin')">
          <div class="logo"></div>微信
        </div>
      </div>
      <div class="login-content-info">
        未注册的手机号验证后将自动创建京东账号,登录即代表您已同意
        <span>京东隐私政策</span>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      isShow: false,
      isActive: false,
      phoneNumber: "",
      checkCodeNumber: "",
      loginWay: "H5",
    };
  },
  watch: {
    checkCodeNumber(val) {
      if (val.trim() !== "" && this.phoneNumber.trim() !== "") {
        this.isActive = true;
      } else {
        this.isActive = false;
      }
    },
    phoneNumber(val) {
      if (val.trim() !== "" && this.checkCodeNumber.trim() !== "") {
        this.isActive = true;
      } else {
        this.isActive = false;
      }
    },
  },
  methods: {
    login(type) {
      this.loginWay = type;
      if (type !== "h5") {
        this.isShow = true;
        return;
      }
      if (this.isActive) {
        this.isShow = true;
      }
    },
    loginCallBack() {
      console.log("login way is " + this.loginWay);
      if (this.loginWay === "h5") {
        let userInfo = JSON.parse(JSON.stringify(this.$store.state.userInfo));
        userInfo.nickName = "JD游客";
        userInfo.userIcon = 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1600769288000&di=67b807afc0d43b432ba94bf455f023d1&imgtype=0&src=http%3A%2F%2Fbpic.588ku.com%2Felement_origin_min_pic%2F01%2F54%2F84%2F575747395b352c9.jpg';
        this.$store.commit("setUserInfo", userInfo);
        this.$router.back();
      } else {
        // h5 plus 代码
        if (window.plus) {
          window.plus.oauth.getServices(
            (services) => {
              let oauthObj = null;
              for (let i = 0; i < services.length; i++) {
                if (services[i].id === this.loginWay) {
                  oauthObj = services[i];
                  break;
                }
              }
              if (oauthObj) {
                // 有平台对象
                if (!oauthObj.authResult || !oauthObj.userInfo) {
                  // 是否登录认证过，没有
                  oauthObj.login(
                    (e) => {
                      // 登录认证
                      let userInfo = e.target.userInfo;
                      let vuexUserInfo = JSON.parse(
                        JSON.stringify(this.$store.state.userInfo)
                      );
                      vuexUserInfo.userIcon = userInfo.headimgurl;
                      vuexUserInfo.nickName = userInfo.nickname;
                      this.$store.commit("setUserInfo", vuexUserInfo); // 保存登录态到本地缓存和vuex
                      window.plus.webview.currentWebview().close();
                      window.history.go(0); // 刷新页面
                    },
                    (e) => {
                      window.plus.nativeUI.toast(
                        "授权失败" + JSON.stringify(e)
                      );
                    }
                  );
                } else {
                  // 登录认证过
                  let userInfo = oauthObj.userInfo;
                  let vuexUserInfo = JSON.parse(
                    JSON.stringify(this.$store.state.userInfo)
                  );
                  vuexUserInfo.userIcon = userInfo.headimgurl;
                  vuexUserInfo.nickName = userInfo.nickname;
                  this.$store.commit("setUserInfo", vuexUserInfo); // 保存登录态到本地缓存和vuex
                  window.plus.webview.currentWebview().close();
                  window.history.go(0); // 刷新页面
                }
              } else {
                window.plus.nativeUI.toast("登录失败");
              }
            },
            () => {
              window.plus.nativeUI.toast("登录失败");
            }
          );
        }
      }
    },
  },
};
</script>
